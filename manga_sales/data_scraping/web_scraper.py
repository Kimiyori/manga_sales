...
class OriconScraper(AbstractScraper):
    ...
    async def fetch(self,
                    ...
                    bs: bool = True) -> BeautifulSoup | ClientResponse:
        """
    ...
    async def _get_title(self, item: BeautifulSoup) -> tuple[str, BeautifulSoup]:
        ...
        try:
            ...
            title_page: BeautifulSoup = await self.fetch(url=mangau_item_link, commands=['content', 'read'])
    ...
    async def _get_image(self, item: BeautifulSoup) -> tuple[str, ClientResponse] | tuple[None, None]:
        try:
    ...
    async def find_latest_date(self,
                               date: datetime.date,
                               operator: Callable[[
