...
class TestOriconScraper(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_image_success(self, mock, mock_uuid):
        text = BeautifulSoup("""<p class="image">
							<span>
    ...
    @patch.multiple('manga_sales.data_scraping.web_scraper.OriconScraper',
                    _get_rating=MagicMock(side_effect=[1, 2]),
                    _get_image=AsyncMock(side_effect=[(
                        'image name1', 'image file1'), (
                        'image name2', 'image file2')]),
                    _get_title=AsyncMock(
                        return_value=('title', 'title_url')),
    ...
    async def test_retrieve_data_success(self, mock_fetch, **mocks):
        mock_fetch.return_value = BeautifulSoup("""<section class="box-rank-entry" itemprop="itemListElement" itemscope="" itemtype="http://schema.org/ListItem">
        ...
        result_data = [Content(name='title', volume=11, image='image name1', imageb='image file1',
                               authors=[], publisher=[], release_date=2022, rating=1, sold=22),
                       Content(name='title', volume=11, image='image name2', imageb='image file2',
                               authors=[], publisher=[], release_date=2022, rating=2, sold=22)]
        self.assertEqual(self.scraper.rating_list, result_data)

    @patch('manga_sales.data_scraping.web_scraper.OriconScraper.fetch')
    async def test_retrive_data_fail_no_class(self, mock_fetch):
        mock_fetch.return_value = BeautifulSoup("""<section  >
        </section><section>
        </section>""",
                                                'html.parser')
        with self.assertRaises(BSError) as error:
    ...
    async def test_get_data_success(self, mock):
        data = [Content(name='title', volume=122, image='image name1', imageb='image file1', authors=[],
                        publisher=[], release_date=2022, rating=1, sold=13),
                Content(name='title', volume=111, image='image name1', imageb='image file1', authors=[],
                        publisher=[], release_date=2022, rating=2, sold=22),
                Content(name='title', volume=113, image='image name1', imageb='image file1', authors=[],
                        publisher=[], release_date=2022, rating=3, sold=45)]
        mock.side_effect = [
