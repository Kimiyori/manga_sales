...
class TestWeek(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncSetUp(self) -> None:
        ...
        self.weeks = [Week(date=datetime.date(2022, 9, 11)), Week(
            date=datetime.date(2022, 6, 6)), Week(date=datetime.date(2021, 8, 22))]
    ...
    async def test_all_group(self):
        async with self.session.get_session as session:
            ...
            self.assertEqual(data, [{'2021': {'August': ['2021-08-22']}},
                                    {'2022': {'June': ['2022-06-06'],
                                              'September': ['2022-09-11']}}])
...
class TestItem(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncSetUp(self) -> None:
        ...
        self.items = [Item(rating=1, volume=22, release_date=datetime.date(2022, 8, 11),
                           sold=11.434, image='2022-08-11/test.jpg'),
                      Item(rating=2, volume=13, release_date=datetime.date(2022, 8, 25),
                           sold=5.234, image='2022-08-11/test2.jpg')]
        async with self.session.get_session as session:
