...
from manga_scrapers.exceptions import ConnectError, NotFound, Unsuccessful
...
TITLE_MATCH_PATTERN_WITH_VOLUME = r"(?P<title>.+)\s(?P<volume>\d+)"
TITLE_MATCH_PATTERN_WITHOUT_VOLUME = r"^(?P<title>[^\[\(]+)\s"
    ...
    def get_page_url(self, list_page: BeautifulSoup) -> str:
        ...
        for i, item in enumerate(items_list):
            ...
            title_match = re.search(
                TITLE_MATCH_PATTERN_WITH_VOLUME
                if self.volume
                else TITLE_MATCH_PATTERN_WITHOUT_VOLUME,
                full_name,
            )
            if not title_match:
                continue
